...
void World::setup(){
	...
	std::srand(9);
	int LiliesCount = std::rand() % (14 - 9) + 9;
	for (int i = 0; i < LiliesCount; i++)
	{
		addEntity(new Lilypad(std::rand() % 800, std::rand() % 800, std::rand() % (110 - 70) + 70));
	}

	int FliesCount = std::rand() % (14 - 9) + 9;
	for (int i = 0; i < FliesCount; i++)
	{
		addEntity(new Fly(std::rand() % 800, std::rand() % 800, std::rand() % (9 - 4) + 4));
	}
}
...
Entity* World::addEntity(Entity* entity){
	...
	else for (std::list<Entity*>::iterator it = entities_.begin(); it != entities_.end(); ++it)
	{
			/*if (std::next(it) == entities_.end())
			{
				entities_.insert(it, entity);
				break;
			}*/
			Entity* currentEntity = (*it);
			if (currentEntity->renderLayer <= entity->renderLayer)
			{
				if (std::next(it) == entities_.end())
				{
					entities_.push_back(entity);
...
void World::actualyDeleteEntities(){
	...
	int i = 0;
	for (it = entitiesToDelete_.rbegin(); it != entitiesToDelete_.rend(); ++it, ++i){
		Entity* currentEntity = (*it);
